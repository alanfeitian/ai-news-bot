...
def summarize_with_deepseek(articles):
    ...
    prompt = f"""以下是从各大媒体抓取过去 24 小时内的所有 AI 新闻列表。请你协助我进行阅读排版，生成一份资讯速递。

要求：
1. **完整保留**：
   - 必须将我给你的**所有新闻**全部列出整理，**不要做任何过滤或遗漏**。
   - 即使有几十条，也请一一列出。无需评判新闻的重要性。

2. **格式排版**：
   - 每条新闻保留原始的核心意思，如果原标题是英文，**必须翻译成中文**标题。
   - 每条新闻的摘要尽量保持在 50-80 字，直接陈述事实即可。
   - 每条新闻之间用 `---` 分割。
   - **去除栏目前缀**：标题中如果包含"氪星晚报｜"、"8点1氪丨"、"晚报｜"、"早报｜"、"晨读｜"等网站栏目名称前缀，必须去除，只保留新闻本身的标题。
   - 格式示例：
...
3. **禁止多余废话**：
   ...
   - 不要写任何“编辑辣评”、“总结点评”之类的结尾段落。通报完最后一条新闻即结束。
